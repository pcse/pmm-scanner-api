...
function parseAPIV1Request(request, response, routedReq) {
	...
	if(keyValuePairs.table == 'students' || keyValuePairs.table == 'events' || keyValuePairs.table == 'general') {
		...
		if(keyValuePairs.table == 'students') {
			...
			mysqlQuery = "SELECT t1.event_id, t2.event_name, t2.semester, t2.year, t3.student_id AS id, t3.first, t3.last, t3.major, t3.year, t3.email, t3.date_added AS since FROM `attendance` AS t1 LEFT JOIN `students` AS t3 ON t1.student_id=t3.student_id LEFT JOIN `events` AS t2 ON t1.event_id=t2.table_name";
		}
